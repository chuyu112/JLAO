...
class AppState:
    ...
    def load_seed_data(self) -> None:
        ...
        from app.repositories import hydrate_state, seed_agent_profiles, seed_products, seed_virtual_customers
        ...
        if product_path.exists():
            seed_products([Product(**item) for item in json.loads(product_path.read_text(encoding="utf-8"))])

        customer_path = SAMPLES_DIR / "virtual_customers.json"
        if customer_path.exists():
            seed_virtual_customers(
                [VirtualCustomer(**item) for item in json.loads(customer_path.read_text(encoding="utf-8"))]
            )

        agent_path = SAMPLES_DIR / "agents.json"
        if agent_path.exists():
            seed_agent_profiles([AgentProfile(**item) for item in json.loads(agent_path.read_text(encoding="utf-8"))])
